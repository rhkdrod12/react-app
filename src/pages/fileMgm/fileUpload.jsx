...
import { makeFileInfo } from "../../utils/commonUtils.js";
import { COM_MESSAGE } from "/src/utils/commonMessage.js";
import { fileRxjsUpload, rxJsPost } from "../../hook/useFetch.jsx";
import { StyleDiv, StyleLi, StyleUl } from "../../module/StyleComp/StyleComp";
import btnClass from "/src/css/Module/Btn.module.css";
import { ReactComponent as Cancel } from "/src/svg/cancel.svg";
import { ReactComponent as Delete } from "/src/svg/delete.svg";
import { useInit } from "../../hook/useReset.jsx";
          ...
          .pipe(
            mergeMap((file) => {
              return of(file).pipe(
                    ...
                    setFileData((files) =>
                      files.map((item) =>
                        item.fileIndex == file.fileIndex
                          ? { ...item, fileTransYn: 1, fileTransPer: percent }
                          : item
                      )
                    );
                  };
                ...
                catchError((error, caught) => {
                  if (
                    error.resultCode === COM_MESSAGE.CANCEL_REQUEST.resultCode
                  ) {
                    setFileData((files) =>
                      files.map((item) =>
                        item.fileIndex == file.fileIndex
        ...
        >
          <StyleDiv
            inStyle={{
              ...
              gridAutoFlow: "column",
              gridColumnGap: 10,
              gridTemplateColumns: "1fr 8fr 2fr 2fr 2fr 3fr",
            }}
          >
...
const FileLi = memo(({ index, fileData, setFileData }) => {
  const onCancleClick = (event) => {
    ...
    setFileData((files) =>
      files.map((item) =>
        item.fileIndex == fileData.fileIndex
          ? { ...item, fileTransYn: 4 }
          : item
      )
    );
  ...
  const onDeleteClick = (event) => {
    setFileData((files) =>
      files.filter((item) => item.fileIndex !== fileData.fileIndex)
    );
    console.log("삭제 요청");
  };
  ...
  return (
    ...
    >
      ...
      >
        {fileData.fileFullName}
      </StyleDiv>
      <StyleDiv inStyle={{ padding: "0px 5px", justifySelf: "right" }}>
        {formatSizeUnits(fileData.fileByte)}
      </StyleDiv>
      <StyleDiv
        inStyle={{
          justifySelf: "center",
      ...
      <StyleDiv inStyle={{ justifySelf: "left" }}>
        {fileData.resultMessage}
      </StyleDiv>
    </StyleLi>
  );
});
