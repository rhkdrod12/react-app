  ...
  return (
    <React.Fragment>
      ...
      <ScrollYBox scrollData={scrollData} ref={ref}>
        <div
          className="grid-data-scrollCotainer"
          style={{ transform: `translateY(${dataTranslateY}px)` }}
        >
          {scrollRowData && scrollRowData.length > 0 ? (
            <GridBody
              rowAllData={scrollRowData}
              rowAction={rowAction}
...
const GridHeaderColumn = memo(({ column }) => {
  ...
  return (
    <StyleDiv
      inStyle={{
        flexBasis: width,
        ...makeDisplayFlexAlign({ verticalAlign, textAlign }),
        ...columnCss,
      }}
      className="grid-column grid-header-column"
    >
...
const GridBodyColumn = memo(({ columnData, Column, rowIdx, colIdx }) => {
  ...
  const param = {
    id,
    data: columnData,
    rowData: rowAction.getRowData(rowIdx),
    rowIdx,
    colIdx,
    rowAction,
    Column,
  ...
  if (Component) {
    ...
    comp = (
      <Component {...param}>
        {dataFormmater?.(columnData) || columnData}
      </Component>
    );
  } else {
  ...
  return (
    <StyleDiv
      inStyle={{
        flexBasis: width,
        ...makeDisplayFlexAlign({ textAlign, verticalAlign }),
        ...columnCss,
      }}
      className="grid-column grid-data-column"
      {...rowEvent}
      {...columnEvent}
    >
      {comp}
...
const makeDefaultValue = (DataInfo, rowAllData) => {
  if (rowAllData && rowAllData.length > 0) {
    const defaultColumn = DataInfo?.Column?.filter(
...
const makeGridParam = (GridInfo) => {
  ...
  if (GridInfo) {
    if (GridInfo.Column) {
      HeaderInfo.Column = copyObjectBykey(
        "id",
        GridInfo.Column,
        HeaderInfo.Column
      );
      DataInfo.Column = copyObjectBykey("id", GridInfo.Column, DataInfo.Column);
      FooterInfo.Column = copyObjectBykey(
        "id",
        GridInfo.Column,
        FooterInfo.Column
      );
    ...
    DataInfo.Scroll = {
      ...DefaultDataInfo.Scroll,
      itemHeight: DataInfo.Row.height,
      headerHeight: HeaderInfo.Row.height,
      ...DataInfo.Scroll,
    };
