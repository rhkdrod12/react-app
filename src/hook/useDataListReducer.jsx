...
const useListDataReducer = (rowAllData) => {
  ...
  const [dataAction, setDataAction] = useState(
    () => new ListDataAction(state, dispatch)
  );
...
const ListDataReducer = (state, action) => {
  ...
  const {
    rowAllData: actionRowAllData = new Array(),
    rowData: actionRowData = {},
    rowIndex: actionRowIndex = -1,
    rowIndexs: actionRowIndexs = new Array(),
  } = action;
  let resultState = state;
  switch (type) {
    case "setInit":
      // rowIndex 부여
      Array.from(actionRowAllData).forEach(
        (item, idx) => (item.rowIndex = idx)
      );
    ...
    case "setRowData":
      ...
      // 데이터 변경처리
      state.rowAllData =
        state.rowAllData.map((val, idx) =>
          idx == actionRowIndex ? actionRowData : val
        ) ?? new Array();
      resultState = state;
      break;
    ...
    case "setColumnData":
      state.rowAllData =
        state.rowAllData.map((val, idx) =>
          idx == actionRowIndex ? { ...val, ...actionRowData } : val
        ) ?? new Array();
      resultState = state;
      break;
    ...
    case "setSelectedRowData":
      state.rowAllData = state.rowAllData.map((val, idx) =>
        idx == state.rowIndex ? actionRowData : val
      );
    ...
    case "addRowData":
      ...
      state.rowAllData.forEach(
        (item) => (state.prevRowAllData[item.rowIndex] = item)
      );
      // rowIndex 부여
          ...
          for (const data of state.filter.filterData) {
            if (
              !compare(
                value,
                data.value,
                compareFunc(data.type ?? COMPARE_STRING.EQUALS)
              )
            ) {
              return false;
            }
    ...
    case "addRowDatas":
      ...
      Array.from(actionRowAllData).forEach(
        (item, idx) => (item.rowIndex = startIndex + idx)
      );
      // prevRowAllData에 현재 추가 값 업데이트
...
/**
 * EQUALS: 값이 서로 같을 때
 * STARTS_WITH: 시작 값이 일치할 때
 * ENDS_WITH : 끝 값이 일치할 때
 * INCLUDES : 값을 포함할 때
 * @type {{EQUALS: string, STARTS_WITH: string, ENDS_WITH: string, INCLUDES: string}}
 */
