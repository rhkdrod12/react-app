...
const useScroll = () => {
  ...
  useEffect(() => {
    ...
    const onScroll = ({ target: { scrollTop } }) => setScrollTop(scrollTop);
 ...
 * @returns
 */
export const useScrollYData = (
  rowData,
  { visibleCount = 5, itemHeight = 50, offsetCnt = 1 } = {}
) => {
  ...
  useEffect(() => {
    setScrollData((data) => {
      ...
      const scrollItemHeight =
        (itemTotalHeight - containerHeight) / (itemTotalCount - visibleCount);
      // 현재 스크롤에서 시작 idx
      const startIdx =
        Math.max(Math.floor(scrollTop / scrollItemHeight), 0) || 0;
      // 현재 스크롤에서 마지막 idx
      ...
      // 표현할 데이터
      const scrollRowData = rowData
        ? rowData.filter((item, idx) => idx >= startIdx && idx < endIdx)
        : null;
      // 스크롤 표기 여부
  ...
  const onScroll = useCallback(({ target: { scrollTop } }) => {
    ...
    setScrollData((data) => {
      ...
      const startIdx = Math.max(
        Math.floor(scrollTop / data.scrollItemHeight),
        0
      );
      // 현재 스크롤에서 마지막 idx
      const endIdx = startIdx + visibleCount + offsetCnt;
      ...
      // 표현할 데이터
      const scrollRowData = data.rowData
        ? data.rowData.filter((item, idx) => idx >= startIdx && idx < endIdx)
        : null;
      // 스크롤 표기 여부
    ...
    return (
      <StyleDiv
        inStyle={{
          position: "relative",
          overflow: "hidden",
          height: scrollData.containerHeight,
        }}
      >
        <StyleDiv inStyle={{ paddingRight: 18, position: "relative" }}>
          {children}
        </StyleDiv>
        {/* Y축 가상 스크롤 */}
        <StyleOverflowDiv
          inStyle={{}}
          className="scroll-virtualY-container"
          height={scrollData.containerHeight}
          ref={ref}
        >
          <StyleScrollDiv height={scrollData.itemTotalHeight}>
            <StyleDiv
              style={{ transform: `translateY(${scrollData.offsetY}px)` }}
            ></StyleDiv>
          </StyleScrollDiv>
        </StyleOverflowDiv>
      </StyleDiv>
...
export const ScrollYBox = memo(
  React.forwardRef(({ children, scrollData }, ref) => {
    return (
      ...
      >
        <StyleDiv
          inStyle={{ paddingRight: 18, position: "relative" }}
        ></StyleDiv>
        {/* Y축 가상 스크롤 */}
        <StyleOverflowDiv
          inStyle={{}}
